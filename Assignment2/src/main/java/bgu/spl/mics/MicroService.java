...
public abstract class MicroService implements Runnable {
    ...
    public MicroService(String name) {
        ...
        this.name = name;
        this.msgToCallback = new HashMap<>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        msgToCallback.putIfAbsent(type,callback);
    ...
    public final void run() {
        ...
        while (!terminated) {
            try {
                Message toHandle = mb.awaitMessage(this);
                if(toHandle != null) {
                    msgToCallback.get(toHandle.getClass()).call(toHandle);
                }
                else {
                    throw new IllegalStateException("The microservice " + this + " is not defined for null messages");
                }
            }
